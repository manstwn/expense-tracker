...
router.get("/money/users", authMiddleware, async (req, res) => {
    try {
...
// 4. Get transactions list with filters
router.get("/money/transactions", authMiddleware, async (req, res) => {
    try {
        const transactions = getTransactions();
        const { userId, search, sortBy = "createdAt", sortOrder = "-1", limit = "100", skip = "0", date } = req.query;

        const query = {};
        
        // Filter by date (YYYY-MM-DD) using Jakarta timezone boundaries
        // "2026-05-18" means May 18 00:00 WIB to May 19 00:00 WIB
        // WIB = UTC+7, so May 18 00:00 WIB = May 17 17:00 UTC
        if (date) {
            const startOfDayJakarta = new Date(`${date}T00:00:00+07:00`);
            if (!isNaN(startOfDayJakarta.getTime())) {
                const endOfDayJakarta = new Date(startOfDayJakarta.getTime() + 24 * 60 * 60 * 1000);
                query.createdAt = {
                    $gte: startOfDayJakarta,
                    $lt: endOfDayJakarta
                };
            }
        }
        ...
        if (userId) {
            ...
            if (!isNaN(parsedId)) {
                query.$or = [{ userId: parsedId }, { userId: userId }];
            } else {
                query.userId = userId;
            }
        }
        ...
        const results = await transactions.find(query)
...
router.get("/money/stats", authMiddleware, async (req, res) => {
    try {
        const transactions = getTransactions();
...
router.post("/money/transactions/parse", authMiddleware, async (req, res) => {
    try {
        const { text } = req.body;
        if (!text) {
...
router.post("/money/transactions", authMiddleware, async (req, res) => {
    try {
        const transactions = getTransactions();
...
router.put("/money/transactions/:id", authMiddleware, async (req, res) => {
    try {
        const transactions = getTransactions();
        const { id } = req.params;
...
router.delete("/money/transactions/:id", authMiddleware, async (req, res) => {
    try {
        const transactions = getTransactions();
        const { id } = req.params;
...
router.get("/money/transactions/top", authMiddleware, async (req, res) => {
    try {
        const transactions = getTransactions();
...
// Add routes for food (/api/food/*) here
